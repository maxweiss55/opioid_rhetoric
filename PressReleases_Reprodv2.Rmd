...
opioid_memcount <- opioid %>%
  ...
  distinct() %>%
  #13,075 Opioid Public Statememnts remain at this point
  #Tally the number of observations grouped by member
  count(member) %>%
  #Arrange from highest to lowest member count
  arrange(desc(n)) %>%
...
```{r Build Sentences Dataset All Public Statements}

#Load in dataset with the date, title, member, path, and text for the 13,500 Opioid Public Statements scraped from VoteSmart on February 18, 2020
opioid <- read_rds('fullscrape.rds') %>%
  #Remove empty rows
  filter(!is.na(date) & !is.na(title) & !is.na(member) & !is.na(path) & !is.na(text)) %>%
  #Filter to include all Public Statements before February 17th
  mutate(date = as.character(date)) %>%
  filter(date != "Feb. 18, 2020") %>%
  #Remove phrase that ended the scraped text of most public statements
  mutate(text = str_remove_all(text, " All content © 1992 - 2020 Vote Smart unless otherwise attributed - Privacy Policy - Legislative demographic data provided by Aristotle International, Inc. Mobile Version \\#\\{text\\} You are about to be redirected to a secure checkout page. Please note: The total order amount will read \\$0.01. This is a card processor fee. Please know that a recurring donation of the amount and frequency that you selected will be processed and initiated tomorrow. You may see a one-time charge of \\$0.01 on your statement. Continue to secure page »")) %>%
  #Remove observations that are full repeats
  distinct()
#13,075 Opioid Public Statements remain at this point
...
opioid_sentences <- opioid %>%
  ...
  mutate(text = str_replace_all(text, "Pres\\.", "Pres")) %>%
  mutate(text = str_replace_all(text, "St\\.", "St")) %>%
  #The two string replacements (U.S. and U.S.A.) below were not identified as signficant during the 116th Congress sentence splitting process but were identified when investigating the full dataset
  mutate(text = str_replace_all(text, "U\\.S\\.", "US")) %>%
  mutate(text = str_replace_all(text, "U\\.S\\.A\\.", "USA")) %>%
  #Separate Public Statements by end punctuation (periods, question marks, exclammation point) and create a new observation for each separated sentence
  separate_rows(text, sep = "[.?!]", convert = FALSE) %>%
  #407,780 split statements derived
  #Filter to only include split statements that contain language directly associated with opioids
  filter(str_detect(text, regex("heroin|opioid|opiate|fentanyl|naloxone|narcan|synthetics|oxycodone|hydrocodone|morphine|codeine|methadone|meperidine|buprenorphine|hydromorphone|benzo", ignore_case = TRUE))) %>%
  #Add a sentence identifier based on row number
  mutate(sentence_id = row_number())
  #60,823 split statements about opioids remain

#Replace the bullet point symbol found in many long lists with a period (prepare for splitting). This is an uncommon string pattern that was a feature of lists and should be treated as separate statements. Split sentences with this string into separate sentences.
opioid_sentences2 <- opioid_sentences %>%
  mutate(text = str_replace_all(text, "·", "."))  %>%
  separate_rows(text, sep = "[.?!]", convert = FALSE) %>%
  mutate(sentence_id = row_number())
#62,266 sentences remain

#Due to the significantly larger size of the full dataset compared to the 116th Congress dataset, sentence splitting was corrected using a simpler workflow (seeking to reduce error without needing to inductively identify all of the potential sentence splitting issues). As this was the case, we simply removed all sentences that were so long that they were identified as outliers (in base R) or so short that they no longer made sense as a full sentence. Therefore, sentences were removed if their length was greater than 414 characters or less than 25 characters.

opioid_sentences2 <- opioid_sentences2 %>%
  mutate(count = str_count(text)) %>%
  filter(count > 24 & count < 415)
#59,429 sentences remain
...
#Save dataframe to .rds file
write_rds(opioid_sentences2, "sentences_FULL.rds")
```
...
```{r 116th Congress Score Public Statement Sentences}
...
write_rds(sent_gram_scores, "sentences_116NA.rds")
...
write_rds(gram_mem_scores, "members_116NA.rds")

```

```{r Full Dataset Score Public Statement Sentences}

#Load in the Public Statement Sentences dataset for scoring
df <- read_rds("sentences_FULL.rds")
#Create the scoring matrix object "sentence_matrix" with the Public Statement Sentences dataset at the argument (input)
#Remember, "Dictionary_ALL.xlsx" must be in the working directory for the function to work.
#Additionally, this function seems to work best after clearing one's R environment for whatever reason (restart R, run function building chunk, run function here)
sentence_matrix <- score_matrix(df)
#If function worked correctly, "sentence_matrix" should contain 79,526 rows.
#Metadata about each Public Statement in Public Statement Sentences dataset fed into the function are retained
...
```{r 116th Congress Public Statement Sentences National Security and MAT Key Words}
...
# #Similarly to the Congressional Drug Dataset Creation, this chunk is commented out because of the potential processing issues when working with the full floor speeches dataset (Gentzkow, Shapiro, and Taddy 2018; downloadable here https://data.stanford.edu/congress_text). Because the workflow associated with the full floor speeches dataset below does not require engaging with the text data, it does not demand as much computing power as the string detection workflow in the Congressional Drug Dataset Creation; therefore, this current chunk has a lower risk of system failure/crashing than the former. 
