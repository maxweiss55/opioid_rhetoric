...
#Build the session tally line graph for a given string.
#To build the graph for a different string or combination of strings, simply exchange for the string indicated to build the `freq` column using regex conventions
drug1 %>%
  filter(!is.na(speakerid) & (party != "N" & party != "I")) %>%
  mutate(date = ymd(date)) %>%
  mutate(freq = ifelse(str_detect(speech, regex("string", ignore_case = TRUE)), 1, 0)) %>%
  group_by(session) %>%
...
#Build the plot for relative frequency of string use over time subset by party (only major parties Republicans "R" and Democrats "D" used)
#To build the graph for a different string or combination of strings, simply exchange for the string indicated to build the `freq` column using regex conventions
drug1 %>%
  filter(!is.na(speakerid) & (party != "N" & party != "I")) %>%
  mutate(date = ymd(date)) %>%
