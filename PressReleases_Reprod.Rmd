...
matrix <- rbind(three, two, one) %>%
  arrange(sentence_id) %>%
  select(sentence_id, word, code, date, member, title, path)
return(matrix)
}
...
#Additionally, this function seems to work best after clearing one's R environment for whatever reason
...
#Load in the full dataset of Congressional Record speeches downloaded from Gentzkow, Shapiro, and Taddy 2018.
...
cr <- cr %>%
    mutate(cocaine = NA_character_) %>%
    mutate(meth = NA_character_) %>%
    mutate(opioid = NA_character_) %>%
    mutate(heroin = NA_character_) %>%
    mutate(fentanyl = NA_character_)
  
for (i in c(1:nrow(cr))) {

  cr$cocaine[i] <- str_count(cr$speech[i], regex("cocaine", ignore_case = TRUE))
  
  if (i %% 100 == 0) {
    print(i)
  }
}
  cr$meth[i] <- str_count(cr$speech[i], regex(" meth |methamphetamine", ignore_case = TRUE))
  cr$opioid[i] <- str_count(cr$speech[i], regex("opioid|opiate", ignore_case = TRUE))
  cr$heroin[i] <- str_count(cr$speech[i], regex("heroin ", ignore_case = TRUE))
  cr$fentanyl[i] <- str_count(cr$speech[i], regex("fentanyl", ignore_case = TRUE))




cr1 <- cr %>%
  filter(row_number() < 1300000)
cr2 <- cr %>%
  filter(row_number() > 1300000 & row_number() < 2600000)
cr3 <- cr %>%
  filter(row_number() > 2600000)
...
cr1 <- cr1 %>%
  mutate(all_opioid = opioid + heroin + fentanyl,
         all_drug_eps = opioid + heroin + fentanyl + cocaine + meth)
...
cr_drug_dataset <- cr_drugs %>%
  filter(all_drug_eps != 0)

write_rds(cr_drug_dataset, "cr_drugs.rds")

cr_memcount_drug <- cr_drug_dataset %>%
  count(member) %>%
  rename(drugs = n)
...
write_rds(cr_memcount, "freq_CR.rds")
