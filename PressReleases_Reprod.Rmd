...
```{r Congressional Record Drug Dataset Creation and Member Frequency Analysis}
#CREATION
...
#Note: This is a very large dataset, due primarily to the large amount of text data. Loading in the original dataset and executing the commands to create the Congressional Record drug speeches subset runs high risk of system failure/crashing. We do not recommend attempting, so we have provided an inclusive confirmation of our drug speeches data below.
...
```{r Score Congressional Record Speeches}

#Load in the Congressional Record Drug Speeches dataset for scoring
df <- read_rds("drug_record.rds") %>%
  filter(session == 114) %>%
  #Rename `speech` variable to `text` to mark as document for scoring in function
  rename(text = speech)
#Add metadata and identifiers to dataset
key <- read_csv("key_CR.csv") %>%
  mutate(speakerid = as.character(speakerid))
df <- df %>%
  left_join(key, by = "speakerid")
#Create the scoring matrix object "sentence_matrix" with the Congressional Record Drug Speeches dataset at the argument (input)
#Remember, "Dictionary_ALL.xlsx" must be in the working directory for the function to work.
#Additionally, this function seems to work best after clearing one's R environment for whatever reason (restart R, run function building chunk, run function here)
#Note: Objects and variables were often named in this workflow referring to each document as "sent" or "sentences" instead of "speech". The purpose of this was to coordinate with function built originally for scoring sentences in the Press Releases.
sentence_matrix <- score_matrix(df)
#If function worked correctly, "speech_matrix" should contain 14,263 rows.
#Metadata about each speech in Congressional Record Drug Speeches dataset fed into the function are retained

#Create dataframe with all speeches labelled by row number for joining in next scoring step
sent_id <- df %>%
  mutate(sentence_id = row_number())
...
#Save file with scored Public Statement sentences
write_rds(sent_gram_scores, "cr_speeches_fullNA.rds")
...
gram_mem_scores <- sent_gram_scores %>%
  ...
  summarise(mem_score = mean(sent_score)) %>%
  #Retrieve all original sentences and join to them member scores (this is only to retrieve the member information, we end up with many redundant rows which will be deleted at the end of piping with distinct() function)
  right_join(sent_id, by = "govtrack_id") %>%
  #Remove members not represented in the sentence scoring
  filter(!is.na(mem_score)) %>%
  #Remove unimportant columns
  select(-date, -text, -speech_id, sentence_id) %>%
  #Only keep one of the duplicated rows for each member
  distinct(govtrack_id, .keep_all = TRUE)

#Save file with the average scores for each member
write_rds(gram_mem_scores, "cr_members_fullNA.rds")
